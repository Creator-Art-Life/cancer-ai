...
interface ModalProps {
  ...
  children: React.ReactNode; // Дочерние элементы, которые будут отображаться внутри модалки
  isOpen: boolean; // Управляет видимостью модалки
  onClose: () => void; // Функция для закрытия модалки
  onAction: () => void; // Функция для выполнения действия при нажатии на кнопку
  actionLabel: string; // Метка для кнопки действия
}
